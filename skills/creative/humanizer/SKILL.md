...
## Voice calibration for team/stakeholder messages

When drafting a message for the user to send to teammates or stakeholders:

- **Terse by default.** Strip all warmup — no "Hey all", no "flagging this", no "I hope this helps", no transitional throat-clearing. Start with the substance.
- **Zero simulated emotion.** No "love that idea", no "excited to share", no "happy to walk anyone through". It reads as fake and reflects poorly. Remove it entirely.
- **State the facts, then the path.** Lead with the concrete finding (what's broken / why it matters), then the recommended action. One pass, no padding.
- **Calibrate length to the ask.** A terse team message = 3–5 sentences max. Don't add context they already have. Don't frame a call to action as helpfulness ("happy to discuss").

**Before (too verbose, fake warmth):**
> Hey all — flagging a dependency issue we should get moving on. We're running `@ngxmc/datetime-picker` which has been unmaintained since Aug 2025 and has an open "Angular 21 support" issue with no resolution. We're on borrowed time with this one. Happy to walk anyone through it.

**After:**
> `@ngxmc/datetime-picker` is unmaintained (last push Aug 2025, open Angular 21 issue unresolved). We need to replace it.
>
> Angular Material v21 ships a native `mat-timepicker` — the path is to wrap them into a shared DS component and migrate existing callsites. Needs a ticket to kick off.
