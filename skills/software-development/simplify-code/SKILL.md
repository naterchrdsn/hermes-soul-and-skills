...
- **NEVER use `execute_code` + `write_file` (hermes_tools) to strip comments from source files.** The `read_file` tool in `hermes_tools` returns content with `LINE_NUM|CONTENT` prefixes (e.g. `1|import {`). If you pass that content to `write_file`, you corrupt the file with line-number prefixes on every line. This caused a TS file to be written as `1|import {\n2|    AfterViewInit,\n...` which broke the entire TypeScript compilation. Use `terminal` + `perl` or `sed` for in-place comment stripping instead:
  ```bash
  # Strip // comments from SCSS/TS (pure comment lines only):
  perl -i -ne 'print unless /^\s*\/\//' file.scss

  # Strip HTML <!-- --> comments (handles multiline):
  perl -0777 -i -pe 's/<!--.*?-->//gs' file.html
  ```
  Only use `execute_code` with `write_file` when you construct the content yourself as a Python string — never when passing content read back from `read_file`.
